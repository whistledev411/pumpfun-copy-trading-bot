...
// Main function with yellowstone gRPC
...
async function main1() {
    ...
    try {
        ...
        const logConnection = new Connection(process.env.RPC_ENDPOINT!, {
            wsEndpoint: process.env.WSS_RPC_ENDPOINT,
            commitment: 'processed'
        });
        ...
        const TARGET_PUB = new PublicKey(TARGET_ADDRESS);
        ...
        globalLogListener = logConnection.onLogs(
            ...
            async ({ logs, err, signature }) => {
                if (err) return
                const isBuy = logs.filter(log => log.includes("Program log: Instruction: Buy")).length;
                if (isBuy) {
                    ...
                    const maker = parsedTransaction.transaction.message.accountKeys[0].pubkey;

                    console.log('Maker => ', maker.toString());

                    const result = await saveToJSONFile('./data.json', parsedTransaction);
        ...
        console.log('Common RPC mode error => ', error);        
    }
}
...
const saveToJSONFile = (filePath: string, data: object): boolean => {
